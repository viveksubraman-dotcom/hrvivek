...
def test_tc_hand_01_equipment_procurement():
    agent = get_orchestrator()
    res = agent.process_message(
        "I am eligible for a home office monitor under remote work policy. Can you order one for me?"
    )
    assert res["status"] == "SUCCESS"
...
def test_tc_hand_02_medical_leave_delegation():
    agent = get_orchestrator()
    res = agent.process_message(
        "I need to take medical leave starting next Monday. Set up my leave and route access."
    )
    assert res["status"] == "SUCCESS"
...
def test_tc_hand_03_relocation_allowance():
    agent = get_orchestrator()
    res = agent.process_message(
        "I am transferring to the London office next month. What is the allowance and badge?"
    )
    assert res["status"] == "SUCCESS"
...
def test_tc_func_04_create_incident_ticket():
    ...
    t = si.create_incident(
        "EMP-90210", "IT-Hardware", "Laptop battery swollen", priority="2 - High"
    )
    assert t["status"] == "New"
...
def test_tc_intg_06_priority_anti_inflation():
    ...
    t = si.create_incident(
        "EMP-90210", "Facilities", "Coffee machine on floor 3 is empty", priority="1 - Critical"
    )
...
def test_tc_gotcha_03_non_remote_equipment_rejection():
    ...
    agent.ww._employees["EMP-11111"] = {
        ...
        "work_location": "Office - SG",
        "home_address": "Singapore",
        "balances": {"vacation_remaining": 40.0},
    }
...
def test_tc_gotcha_04_extended_bereavement():
    agent = get_orchestrator()
    res = agent.process_message(
        "What is the bereavement leave for extended family like grandparents?"
    )
...
def test_tc_bait_06_unmatched_policy_abstention():
    agent = get_orchestrator()
    res = agent.process_message(
        "What is the company policy on bringing private hoverboards to the office?"
    )
