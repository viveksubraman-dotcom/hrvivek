...
def calculate_working_days(start_str: str, end_str: str) -> int:
    ...
    if start_dt > end_dt:
        raise ValueError(
            f"Temporal validity violation: start_date ({start_str}) cannot be after end_date ({end_str})"
        )
...
def validate_leave_request(
    start_str: str, end_str: str, available_days: float
    ...
    if start_dt > end_dt:
        return (
            False,
            f"Invalid date range: start_date ({start_str}) is after end_date ({end_str}).",
            0,
        )
    ...
    if working_days == 0:
        return (
            False,
            "Invalid leave request: Selected dates contain 0 working business days (e.g., weekends only).",
            0,
        )
    ...
    if working_days > available_days:
        return (
            False,
            f"Insufficient leave balance: requested {working_days} working days, but accrued balance is only {available_days} days.",
            working_days,
        )
...
def validate_leave_increment(requested_days: float) -> Tuple[bool, Optional[str]]:
    ...
    if requested_days <= 0:
        return (
            False,
            f"Invalid duration: Requested leave days must be greater than 0 (got {requested_days}).",
        )
    if (requested_days * 2) % 1 != 0:
        return (
            False,
            f"Invalid increment: Leave must be requested in full or half-day increments (0.5 days), got {requested_days}.",
        )
    return True, None
...
def validate_ticket_transition(
    ...
    if target_status not in allowed:
        return (
            False,
            f"Illegal lifecycle transition: Cannot transition incident directly from '{current_status}' to '{target_status}'. Allowed transitions: {allowed}.",
        )
    return True, None
