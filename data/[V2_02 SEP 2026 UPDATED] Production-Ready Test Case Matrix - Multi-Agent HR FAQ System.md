...
* **Evaluation Scenarios**:  
  - `TC-ROUT-01` / `TC-POL-01`: Bereavement Leave entitlement (up to 4 weeks / 20 work days per event within 12 months per Section 22 & Section 3.1; pet loss excluded).  
...
| :---- | :---: | :---- | :---- | :---- | :---- | ----- | :---: |
| **TC-ROUT-01** | Tier 1 | Routing | Policy Intent: Bereavement Leave | *"What is the company bereavement leave policy for loss of an immediate family member?"* | `rag_agent(query)` $\\rightarrow$ Quotes up to 4 weeks (20 work days for a standard 5-day schedule) paid leave per event, usable within 12 months, with deep link to Section 22\. | `traj`: 1.0 `match`: 0.96 `cite`: 2.0 | `PASSED` |
...
| **TC-POL-01** | Tier 1 | Policy | Ground Truth: Bereavement Entitlement | *"How much bereavement leave am I entitled to for the loss of a parent, and what is the usage timeline?"* | Returns up to 4 weeks (20 work days for a 5-day schedule) of paid leave per event, taken within 12 months of the death, citing Section 22.2 & Section 3.1. | `ground`: 1.0 `match`: 0.99 `cite`: 2.0 | `PASSED` |
...
| **TC-HAND-04** | Tier 2 | MAS Gotcha | Multi-Domain: Maternity & Stock Vesting | *"How many weeks of paid maternity leave do I get in Singapore, and does my stock continue to vest during the leave?"* | Chained: `rag_agent` (retrieves 24 weeks standard paid maternity leave per Section 27.2, extendable to 26 weeks via SPL) $\\rightarrow$ `benefits_agent` (confirms stock vesting continues as normal per Section 27.5). | `traj`: 1.0 `reasoning`: 2.0 | `PASSED` |
